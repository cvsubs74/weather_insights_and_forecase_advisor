...
const RiskAnalysis = () => {
  ...
  const handleSearchAlerts = async (e) => {
    ...
    try {
      ...
      if (response) {
        // Check if we have structured alerts from state
        if (response.alerts && Array.isArray(response.alerts) && response.alerts.length > 0) {
          console.log('[RiskAnalysis] Using structured alerts from state:', response.alerts);
          const structuredAlerts = response.alerts.map((alert, index) => ({
            id: index,
            event: alert.event || 'Unknown Event',
            headline: alert.headline || 'No details available',
            severity: alert.severity || 'Unknown',
            fullText: `${alert.event}: ${alert.headline}`
          }));
          setAlerts(structuredAlerts);
        } else if (response.content) {
          // Check if response indicates no alerts
          const content = response.content.toLowerCase();
          const noAlertsIndicators = [
            'no active alerts',
            'no weather alerts',
            'no current alerts',
          ...
          const hasNoAlerts = noAlertsIndicators.some(indicator => content.includes(indicator));
          
          if (hasNoAlerts) {
            setAlerts([]);
            setRiskAnalysis(response.content);
          } else {
            // Try to parse the new format: "Event Name (Severity): Description"
            const alertPattern = /^(.+?)\s*\((\w+)\s+severity\):\s*(.+)$/gm;
            const matches = [...response.content.matchAll(alertPattern)];
            
            if (matches.length > 0) {
              console.log('[RiskAnalysis] Parsed alerts from text:', matches);
              const parsedAlerts = matches.map((match, index) => ({
                id: index,
                event: match[1].trim(),
      ...
      {riskAnalysis && !loadingAnalysis && (
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="prose max-w-none">
            <ReactMarkdown
              components={{
                h1: ({node, children, ...props}) => <h1 className="text-2xl font-bold text-gray-900 mb-4 mt-6" {...props}>{children}</h1>,
                h2: ({node, children, ...props}) => <h2 className="text-xl font-semibold text-gray-800 mb-3 mt-5" {...props}>{children}</h2>,
                h3: ({node, children, ...props}) => <h3 className="text-lg font-semibold text-gray-700 mb-2 mt-4" {...props}>{children}</h3>,
